    ...
    <div class="post" v-if="posts">
      <div class="card user-post" v-for="post in posts" :key="post.id">
        ...
        <div class="card-content post-info">
          ...
          <a class="btn grey lighten-2 btn-follow" :class="{'pink-text' : post.has_been_followed}"
          ...
          <span class="card-title activator grey-text text-darken-4 user-name">
            <router-link :to="{ name: 'UserProfile', params: {id: post.user_slug}}">
              <img class="av" :src="post.ava">
            </router-link>
            {{ post.userName }}
          </span>
          ...
          <a class="btn grey lighten-2 btn-like" :class="{'pink-text' : post.has_been_liked}"
